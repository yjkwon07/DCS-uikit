...
const colorStyle = ({ background, color }: StyledColorProps) => {
  return css`
    background: ${
      background && get(colors, background.replace(' !important', ''))
        ? get(colors, background.replace(' !important', '')) + background.includes('!important') && ' !important'
        : background
    }};
    color: ${
      color && get(textColor, color.replace(' !important', ''))
        ? get(colors, color.replace(' !important', '')) + color.includes('!important') && ' !important'
        : color
    }};
  `;
};
